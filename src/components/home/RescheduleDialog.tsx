...
interface RescheduleDialogProps {
  ...
  currentDate: string | string[] | Date[] | null;
...
export default function RescheduleDialog({
    ...
    try {
      ...
      if (Array.isArray(currentDate)) {
        if (currentDate.length > 0) {
          if (currentDate[0] instanceof Date) {
            return currentDate as Date[];
          } else {
            // Handle string[] type
            return (currentDate as string[]).map(dateStr => {
  ...
  // Handle time in format "10:00" or numeric
  const getInitialTime = () => {
    if (!currentTime) return "09:00";
    
    if (typeof currentTime === 'string') {
      // Check if the time is in the format "10:00"
