
import { useState } from "react";
import { useAdminUsers } from "@/hooks/useAdminUsers";
import { toast } from "sonner";
import UserTable from "./users/UserTable";
import UserListControls from "./users/UserListControls";
import UserPagination from "./users/UserPagination";
import UserAddressDialog from "./users/UserAddressDialog";
import UserRankUpdateDialog from "./users/UserRankUpdateDialog";
import NotificationModal from "./users/NotificationModal";
import { UserWithReservations } from "@/types/admin";
import { Button } from "@/components/ui/button";
import { Bell, Download, FileText, File, Upload } from "lucide-react";
import { exportUsers } from "@/utils/exportUtils";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  DropdownMenuTrigger,
} from "@/components/ui/dropdown-menu";
import CSVUploadDialog from "./shared/CSVUploadDialog";
import { saveUsersToDatabase } from "@/utils/csvUtils";

const UserList = () => {
  const [addressDialogOpen, setAddressDialogOpen] = useState(false);
  const [selectedUserForAddress, setSelectedUserForAddress] = useState<UserWithReservations | null>(null);
  const [notificationModalOpen, setNotificationModalOpen] = useState(false);
  const [csvUploadDialogOpen, setCsvUploadDialogOpen] = useState(false);
  
  const {
    searchTerm,
    setSearchTerm,
    currentPage,
    setCurrentPage,
    sortField,
    sortOrder,
    handleSort,
    paginatedUsers,
    totalPages,
    isLoading,
    error,
    selectedUsers,
    handleSelectUser,
    handleSelectAll,
    rankUpdateDialogOpen,
    setRankUpdateDialogOpen,
    selectedUser,
    openRankUpdateDialog,
    refetch,
    filteredUsers
  } = useAdminUsers();

  const handleAddressView = (user: UserWithReservations) => {
    setSelectedUserForAddress(user);
    setAddressDialogOpen(true);
  };

  const handleExport = (format: 'csv' | 'pdf') => {
    const dataToExport = selectedUsers.length > 0 
      ? filteredUsers.filter(user => selectedUsers.includes(user.id))
      : filteredUsers;
      
    if (dataToExport.length === 0) {
      toast("내보낼 데이터가 없습니다.");
      return;
    }
    
    exportUsers(dataToExport, format);
    toast.success(`${dataToExport.length}명의 사용자 데이터를 ${format.toUpperCase()} 형식으로 내보냈습니다.`);
  };

  const handleBulkUpload = async (users: UserWithReservations[]) => {
    try {
      toast.info(`${users.length}명의 사용자 정보를 저장하는 중...`);
      const result = await saveUsersToDatabase(users);
      
      if (result.success > 0) {
        toast.success(`${result.success}명의 사용자 정보가 성공적으로 저장되었습니다.`);
        refetch(); // Refresh the user list
      }
      
      if (result.failed > 0) {
        toast.error(`${result.failed}명의 사용자 정보 저장에 실패했습니다.`);
      }
    } catch (error) {
      toast.error(`사용자 정보 저장 중 오류가 발생했습니다: ${error instanceof Error ? error.message : String(error)}`);
    }
  };

  if (error) {
    console.error("Error loading users:", error);
    return <div className="text-red-500">사용자 목록을 불러오는 중 오류가 발생했습니다.</div>;
  }

  console.log(paginatedUsers);
  return (
    <div className="space-y-4">
      <div className="flex justify-end items-center gap-2">
        <Button 
          variant="outline"
          className="flex items-center gap-2"
          onClick={() => setCsvUploadDialogOpen(true)}
        >
          <Upload className="h-4 w-4" />
          CSV 업로드
        </Button>
        
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button 
              variant="outline"
              className="flex items-center gap-2"
            >
              <Download className="h-4 w-4" />
              내보내기
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={() => handleExport('csv')}>
              <File className="h-4 w-4 mr-2" />
              CSV로 내보내기
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => handleExport('pdf')}>
              <FileText className="h-4 w-4 mr-2" />
              PDF로 내보내기
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
        
        <Button 
          className="flex items-center gap-2"
          onClick={() => setNotificationModalOpen(true)}
        >
          <Bell className="h-4 w-4" />
          알림 전송
        </Button>
      </div>

      <UserListControls
        searchTerm={searchTerm}
        onSearchChange={setSearchTerm}
        sortField={sortField}
        sortOrder={sortOrder}
        onSortFieldChange={(field) => handleSort(field)}
        onSortOrderChange={(order) => handleSort(sortField)}
        selectedUsersCount={selectedUsers.length}
        onBulkAction={() => {
          toast(`${selectedUsers.length}명의 사용자가 선택되었습니다.`);
        }}
        totalUsers={paginatedUsers.length}
      />
      {/* Todo: right go creat a new  view for east access add count. */}
      <UserTable
        users={paginatedUsers}
        isLoading={isLoading}
        selectedUsers={selectedUsers}
        onSelectUser={handleSelectUser}
        onSelectAll={handleSelectAll}
        onAddressView={handleAddressView}
        onRankUpdate={openRankUpdateDialog}
        sortField={sortField}
        sortOrder={sortOrder}
        onSort={handleSort}
        searchTerm={searchTerm}
      />
      
      {totalPages > 1 && (
        <UserPagination
          currentPage={currentPage}
          totalPages={totalPages}
          onPageChange={setCurrentPage}
        />
      )}
      
      <UserAddressDialog
        open={addressDialogOpen}
        onOpenChange={setAddressDialogOpen}
        user={selectedUserForAddress}
      />
      
      <UserRankUpdateDialog
        open={rankUpdateDialogOpen}
        onOpenChange={setRankUpdateDialogOpen}
        user={selectedUser}
        onRankUpdated={refetch}
      />

      <NotificationModal
        open={notificationModalOpen}
        onOpenChange={setNotificationModalOpen}
      />
      
      <CSVUploadDialog
        open={csvUploadDialogOpen}
        onOpenChange={setCsvUploadDialogOpen}
        onUpload={handleBulkUpload}
        type="user"
      />
    </div>
  );
};

export default UserList;
