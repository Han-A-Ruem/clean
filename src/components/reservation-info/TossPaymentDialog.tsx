...
export function TossPaymentDialog() {
  ...
  // Load Toss Payments SDK once when component mounts
  useEffect(() => {
    async function fetchPaymentWidgets() {
      try {
        ...
        const loadedWidgets = tossPayments.widgets({
          customerKey,
        });
        // 비회원 결제
  ...
  // Render widgets when dialog is opened and widgets are loaded
  useEffect(() => {
    async function renderPaymentWidgets() {
      if (!isOpen || !widgets || !tossLoaded) {
        return;
      }
      
      setReady(false);
      
      // Wait a short time to ensure the dialog content is rendered in the DOM
      setTimeout(async () => {
        try {
          // Check if the DOM elements exist before rendering
          const paymentMethodElement = document.getElementById('payment-method');
          const agreementElement = document.getElementById('agreement');
          
          if (!paymentMethodElement || !agreementElement) {
            console.error('Payment method or agreement elements not found in DOM');
            return;
          }
  ...
  }, [isOpen, widgets, tossLoaded, amount]);
